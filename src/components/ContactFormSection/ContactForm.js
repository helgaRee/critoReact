  ...
  const handleForm = async (e) => {
    ...
    if (!nameError && !emailError && !messageError) {
      ...
      if (result.status === 200) {
        ...
        setErrorMessage(
          "Hmm, något gick fel.. du verkar inte fyllt i alla fält."
        )
        console.log(result.status) //kontroll
      } else {
    ...
    // sätt specifikt meddelande om mindre än 3 fält är korrekt ifyllda
      ...
      >
        <div className="mb-3">
          <label className={`${nameError ? "errorMessage" : ""}`}>{`Namn ${
            nameError ? " måste anges med minst 2 tecken" : ""
          }`}</label>
          <input
        ...
        <div className="mb-3">
          <label className={`${emailValid ? "" : "errorMessage"}`}>
            {" "}
            {`Din Email ${emailValid ? "" : " måste vara giltig "}`}
          </label>
          <input
            onChange={(e) => handleChange(e)}
            value={email}
        ...
        <div className="mb-5">
          <label className={`${messageError ? "errorMessage" : ""}`}>
            {" "}
            {`${messageError ? "Glöm inte att lämna ett meddelande" : ""}`}
          </label>
          <textarea
            onChange={(e) => handleChange(e)}
            value={message}
